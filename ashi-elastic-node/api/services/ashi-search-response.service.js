...
module.exports = {
    getSearchSummaryResponseInAshiFormat,
    getSearchResponseInAshiFormat
}
...
function prepareProductListResponse(blankProductList, ashiRequest, elasticData) {
    ...
    blankProductList["request"] = ashiRequest
...
async function getAttribValues(inputData, ashiRequest) {
    let returnData = [];
    globalSettings.pages["search-product-list-page"].side_filters.filter_mapping.forEach(globalsettingElement => {
...
function fillAttribOptions(data, attribName, ashiRequest, globalsettingElement) {
    ...
    if (
        (data.attribute_aggs &&
            ...
            data.attribute_aggs.filtered_docs[attribName].buckets) ||
        (data.category_info &&
            data.category_info.filtered_docs
        )
    ) {
...
function fillCategoryCounts(inputData, ashiRequest) {
    ...
    let categoryCounts = getCategoryCounts(inputData);
    categoryCountObject.subcategory = categoryCounts;
    return categoryCountObject;
}

function getCategoryCounts(inputData) {
    data = inputData.dataArray.aggregations.attribute_aggs.filtered_docs;
    buckets = data.category_count.buckets;
    let returnData = buckets.map(bucket => ({
        categoryname: bucket.key["category_count"],
        count: bucket.doc_count,
        order: bucket.key.seq_num,
        categoryid: 0,
        oldcategoryid: 0,
...
function getProducts(elasticData) {
    ...
    elasticData.hits.hits.forEach(element => {
        ...
        productElement["price_retail"] = element.inner_hits.retailer_data.hits.hits[0]._source.Price_Retail;
        //productElement["sprice"] = element.inner_hits.retailer_data.hits.hits[0]._source.Price;
...
function getSearchResponseInAshiFormat(ashiRequest, elasticData) {

    let productListResponse = getProductResponseInAshiFormat(ashiRequest, elasticData);
    return productListResponse;
}

// async function getSearchSummaryResponseInAshiFormat(elasticData, ashiRequest) {
//     let header = [{
//         "title": "Product Type",
//         "order": 0,
//         "categorycounts": []
...
//     summary["attrib"] = await getAttribValues(elasticData);
//     data = elasticData;
...
async function getSearchSummaryResponseInAshiFormat(elasticData, ashiRequest) {
    ...
    summary["header"] = header;
    summary["attrib"] = await getAttribValues(elasticData, ashiRequest);
    data = elasticData;
    return summary
}
